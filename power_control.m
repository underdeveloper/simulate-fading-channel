function processed_signal = power_control(received_signal, symbol_rate, limit, step_size, control_sample_rate)
arguments
    ...
    step_size double {mustBeNumeric} % How much the power control has control over the signal, in dBmW
...
%             disp(['Iteration: ' num2str(iterations) ', at ' num2str(10*log10(abs(processed_signal(m))/1e-3))]);
            processed_signal(m) = abs(processed_signal(m)) * 10^(step_size/10) * exp(1i*angle(processed_signal(m)));
            % note an *increase* in X dBm is the same as an increase in X
            % dBW, they're both dB.
%             processed_signal(m) =
%             10^(3+((10*log10(abs(processed_signal(m))/1e-3) + ...
%             step_size)/10)) * exp(angle(processed_signal(m))); % this
%             didnt work
